...
public:
  ...
  TimerHandler(TimerId timer_id, OnMessage timeout_call_back,
               Callable call_back)
      : timer_id_(timer_id), timeout_call_back_{timeout_call_back},
        call_back_{call_back} {}
  void Arm(Duration duration) {
    std::unique_lock<std::mutex> lock{mtx};
    is_stopped = false;
    auto time_ = std::chrono::system_clock::now() + duration;
    cv.wait_until(lock, time_, [&] { return is_stopped.load(); });
  ...
  Callable GetCallBack() { return call_back_; }
